...
import { IconEyeCog, IconPlus } from "@tabler/icons-react";
...
import customer_data from "./CustomerData.json";
...
export default function AppointmentTable() {
  ...
  const customers: Customer[] = React.useMemo(
    () =>
      customer_data.map((customer, index) => ({
        ...customer,
        representative: representatives[index % representatives.length],
      })),
    [representatives],
  );
  ...
  const onGlobalFilterChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const globalFilter = _filters["global"] as {
      value: string | null;
      matchMode: string;
    };
  ...
  const actionBodyTemplate = () => {
    return <Button type="button" leftSection={<IconEyeCog />} />;
  };
  ...
  return (
    <div className="card" style={{ overflow: "auto" }}>
      ...
      <AppointmentModal opened={opened} close={close} />
    </div>
  );
}
